...
export class FirebaseUpdaterAndSetterService {
  ...
  setAntenna(antenna: Antenna) {
    this.auth.currentUser.then(user => {
      this.afDatabase.database.ref('users/'+user.uid+'/antennae').push(antenna).then((r) => { this.afDatabase.database.ref('users/'+user.uid+'/antennae/' + r.key).update({ id: r.key }) });
  ...
  setRadioSet(radioset: RadioSet) {
    this.auth.currentUser.then(user => {
      this.afDatabase.database.ref('users/'+user.uid+'/equipment').push(radioset).then((r) => { this.afDatabase.database.ref('users/'+user.uid+'/equipment/' + r.key).update({ id: r.key }) });
  ...
  testHTML() {
    ...
    console.log(this.http.get<any>('http://90.162.39.97:3001/list/Málaga', { headers }));
    this.http.get<any>('http://90.162.39.97:3001/list/Málaga', { headers }).subscribe(val => {
      if (val.length === 0) {
        console.log("empty")
      } else {
        console.log(val);
    ...
    this.http.get<any>('http://90.162.39.97:3001/list/Málaga', {}).subscribe(val => {
      if (val.length > 0) {
        console.log(val[0].coordenada);
      } else {
        this.http.get<any>('http://90.162.39.97:3001/cani', {}).subscribe(data => {
          console.log(data[0]);
          if (data[0].issearching === 0) {
            ...
            const params = new HttpParams();
            this.http.get<any>('')
            this.http.post<any>('http://90.162.39.97:3001/operatecon/', {}).subscribe(data => {
              console.log(data);
              setTimeout(() => {
                this.http.post<any>('http://90.162.39.97:3001/operatecon/', {}).subscribe(data => {
                  console.log(data);
  ...
  async setContact(contact: Contact, audio: File) {
    if (contact.location === null) {
      ...
      this.http.get<any>('http://90.162.39.97:3001/list/' + contact.location, {}).subscribe(val => {
        if (val.length > 0) {
          ...
          this.http.get<any>('http://90.162.39.97:3001/cani', {}).subscribe(data => {
            ...
            if (data[0].issearching === 0) {
              ...
              this.http.get<any>('https://nominatim.openstreetmap.org/search.php?q='+contact.location+'&email=pablo.molina@iescampanillas.com&format=json&limit=1',{headers}).subscribe(data => {
                ...
                this.http.post<any>('http://90.162.39.97:3001/newcoordinate/', { "coordenada": data[0].lat + "," + data[0].lon, "terminobusqueda": contact.location }).subscribe(data => {
                  this.http.post<any>('http://90.162.39.97:3001/operatecon/', {}).subscribe(data => {
                    console.log(data);
                    setTimeout(() => {
                      this.http.post<any>('http://90.162.39.97:3001/operatecon/', {}).subscribe(data => {
  ...
  deleteContact(contact: Contact) {
      ...
      this.auth.currentUser.then(user => {
        this.afDatabase.database.ref('users/'+user.uid+'/contacts/'+contact.id).remove().then(c => {
          this.auth.currentUser.then(user => {
            this.storage.ref(user.uid).child("test")
